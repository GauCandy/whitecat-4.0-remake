  ...
  async execute(message: Message): Promise<void> {
    try {
      ...
      if (mentionedUsers.length === 0) {
        await message.reply({
          content: '❌ Please mention someone to call baka! Example: `,baka @user`',
        });
        return;
      }
      ...
      if (targets.length === 0) {
        await message.reply({
          content: `😤 **${message.author.username}** calls themselves baka... Self-roast!`,
        });
        return;
      }

      const loadingMsg = await message.reply('😤 Calling someone baka...');
      const gifUrl = await getNekobest(NekobestAction.Baka);

      if (targets.length > 1) {
        const targetNames = targets.map(u => `**${u.username}**`).join(', ');
        const messages = [
          `😤 **${message.author.username}** calls ${targetNames} baka! Tsundere mode activated!`,
          `💢 BAKA BAKA BAKA! **${message.author.username}** yells at ${targetNames}! Everyone's an idiot!`,
          `😡 **${message.author.username}** goes full tsundere and calls ${targetNames} baka! Hmph!`,
        ];
        ...
        const embed = new EmbedBuilder()
          ...
          .setImage(gifUrl)
          .setFooter({
            text: `Tsundere energy by ${message.author.username}`,
      ...
      const embed = new EmbedBuilder()
        ...
        .setImage(gifUrl)
        .setFooter({
          text: `${target.username} got called baka by ${message.author.username}`,
          iconURL: message.author.displayAvatarURL(),
        })
        .setTimestamp();
      ...
      logger.error('Error in baka text command:', error);
