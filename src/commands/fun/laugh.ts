...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('laugh')
        .setDescription(t(Locale.English, 'commands.fun.laugh.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.laugh.description'),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Laugh);

            // Get random message from array
            const messages = t(locale, 'commands.fun.laugh.message');
