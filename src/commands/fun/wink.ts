...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('wink')
        .setDescription(t(Locale.English, 'commands.fun.wink.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.wink.description'),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Wink);

            // Get random message from array
            const messages = t(locale, 'commands.fun.wink.message');
