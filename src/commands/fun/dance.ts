...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('dance')
        .setDescription(t(Locale.English, 'commands.fun.dance.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.dance.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Dance);

            // Get random message from array
            const messages = t(locale, 'commands.fun.dance.message');
