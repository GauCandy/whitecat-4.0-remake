...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('feed')
        .setDescription(t(Locale.English, 'commands.fun.feed.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.feed.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            if (targetUser.id === interaction.user.id) {
                const selfMessages = t(locale, 'commands.fun.feed.self');
            ...
            logger.error('Error in feed command:', error);
