...
const feedCommand: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option
        .setName('target')
        .setDescription('The person to feed')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      const gifUrl = await nekosService.getRandomGif('feed');
      const message = await localeService.tGuild(
        interaction.guildId,
        'commands.fun.feed.message',
        {
          user: interaction.user.toString(),
