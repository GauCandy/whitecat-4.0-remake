...
const nomCommand: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option
        .setName('target')
        .setDescription('The person to nom')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      const gifUrl = await nekosService.getRandomGif('nom');
      const message = await localeService.tGuild(
        interaction.guildId,
        'commands.fun.nom.message',
        {
          user: interaction.user.toString(),
