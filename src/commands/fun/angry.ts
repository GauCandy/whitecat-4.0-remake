...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('angry')
        .setDescription(t(Locale.English, 'commands.fun.angry.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.angry.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Angry);

            // Get random message from array
            const messages = t(locale, 'commands.fun.angry.message');
            ...
            logger.error('Error in angry command:', error);
