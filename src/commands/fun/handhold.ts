...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('handhold')
        .setDescription(t(Locale.English, 'commands.fun.handhold.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.handhold.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            if (targetUser.id === interaction.user.id) {
                const selfMessages = t(locale, 'commands.fun.handhold.self');
