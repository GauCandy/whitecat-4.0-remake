...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('run')
        .setDescription(t(Locale.English, 'commands.fun.run.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.run.description'),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Run);

            // Get random message from array
            const messages = t(locale, 'commands.fun.run.message');
