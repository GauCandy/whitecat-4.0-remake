...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('smug')
        .setDescription(t(Locale.English, 'commands.fun.smug.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.smug.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Smug);

            // Get random message from array
            const messages = t(locale, 'commands.fun.smug.message');
