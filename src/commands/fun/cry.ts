...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('cry')
        .setDescription(t(Locale.English, 'commands.fun.cry.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.cry.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Cry);

            // Get random message from array
            const messages = t(locale, 'commands.fun.cry.message');
