...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('happy')
        .setDescription(t(Locale.English, 'commands.fun.happy.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.happy.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Happy);

            // Get random message from array
            const messages = t(locale, 'commands.fun.happy.message');
