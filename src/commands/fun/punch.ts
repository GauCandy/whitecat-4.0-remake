...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('punch')
        .setDescription(t(Locale.English, 'commands.fun.punch.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.punch.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            if (targetUser.id === interaction.user.id) {
                const selfMessages = t(locale, 'commands.fun.punch.self');
