...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('peck')
        .setDescription(t(Locale.English, 'commands.fun.peck.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.peck.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            if (targetUser.id === interaction.user.id) {
                const selfMessages = t(locale, 'commands.fun.peck.self');
            ...
            logger.error('Error in peck command:', error);
