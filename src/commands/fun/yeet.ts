...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('yeet')
        .setDescription(t(Locale.English, 'commands.fun.yeet.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.yeet.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            if (targetUser.id === interaction.user.id) {
                const selfMessages = t(locale, 'commands.fun.yeet.self');
            ...
            const gifUrl = await getNekobest(NekobestAction.Yeet);

            // Get message (handle bot case or normal)
            const messageKey = isBot ? 'commands.fun.yeet.bot' : 'commands.fun.yeet.message';
