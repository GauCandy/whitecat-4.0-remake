...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('sleep')
        .setDescription(t(Locale.English, 'commands.fun.sleep.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.sleep.description'),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Sleep);

            // Get random message from array
            const messages = t(locale, 'commands.fun.sleep.message');
