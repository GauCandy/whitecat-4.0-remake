...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('highfive')
        .setDescription(t(Locale.English, 'commands.fun.highfive.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.highfive.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            if (targetUser.id === interaction.user.id) {
                const selfMessages = t(locale, 'commands.fun.highfive.self');
