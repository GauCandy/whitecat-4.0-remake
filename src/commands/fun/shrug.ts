...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('shrug')
        .setDescription(t(Locale.English, 'commands.fun.shrug.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.shrug.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Shrug);

            // Get random message from array
            const messages = t(locale, 'commands.fun.shrug.message');
