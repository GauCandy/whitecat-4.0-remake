...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('wave')
        .setDescription(t(Locale.English, 'commands.fun.wave.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.wave.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Wave);

            // Get random message from array
            const messages = t(locale, 'commands.fun.wave.message');
