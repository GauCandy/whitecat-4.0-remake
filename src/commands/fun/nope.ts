...
const command: Command = {
    data: buildLocalizedCommand('nope', 'fun')
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Nope);

            // Get random message from array
            const messages = t(locale, 'commands.fun.nope.message');
            ...
            logger.error('Error in nope command:', error);
