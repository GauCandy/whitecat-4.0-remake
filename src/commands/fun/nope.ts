...
const nopeCommand: SlashCommand = {
  data: new SlashCommandBuilder()
    .setName('nope')
    .setDescription('Refuse or disagree')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      const gifUrl = await nekosService.getRandomGif('nope');
      const message = await localeService.tGuild(
        interaction.guildId,
        'commands.fun.nope.message',
