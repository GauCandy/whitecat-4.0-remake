...
const yawnCommand: SlashCommand = {
  data: new SlashCommandBuilder()
    .setName('yawn')
    .setDescription('Yawn tiredly')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      const gifUrl = await nekosService.getRandomGif('yawn');
      const message = await localeService.tGuild(
        interaction.guildId,
        'commands.fun.yawn.message',
