...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('facepalm')
        .setDescription(t(Locale.English, 'commands.fun.facepalm.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.facepalm.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Facepalm);

            // Get random message from array
            const messages = t(locale, 'commands.fun.facepalm.message');
