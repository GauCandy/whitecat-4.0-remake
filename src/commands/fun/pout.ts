...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('pout')
        .setDescription(t(Locale.English, 'commands.fun.pout.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.pout.description'),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Pout);

            // Get random message from array
            const messages = t(locale, 'commands.fun.pout.message');
