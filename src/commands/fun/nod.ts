...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('nod')
        .setDescription(t(Locale.English, 'commands.fun.nod.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.nod.description'),
        })
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Nod);

            // Get random message from array
            const messages = t(locale, 'commands.fun.nod.message');
