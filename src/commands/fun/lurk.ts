...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('lurk')
        .setDescription(t(Locale.English, 'commands.fun.lurk.description'))
        .setDescriptionLocalizations({
            vi: t(Locale.Vietnamese, 'commands.fun.lurk.description'),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        try {
            ...
            const gifUrl = await getNekobest(NekobestExpression.Lurk);

            // Get random message from array
            const messages = t(locale, 'commands.fun.lurk.message');
            ...
            logger.error('Error in lurk command:', error);
